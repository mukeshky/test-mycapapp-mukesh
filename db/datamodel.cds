...
context master {
    ...
    entity product{
        ...
        TYPE_CODE: String(2);
        CATEGORY: String(32);
        DESCRIPTION: localized String(255) @(title: '{i18n>NAME}');
...
context transaction {
    entity purchaseorder: common.Amount, cuid{
        //key NODE_KEY: common.Guid @(title: '{i18n>PO_ID_NODE_KEY}');
    ...
    entity poitems: common.Amount, cuid{
        // NODE_KEY: common.Guid @(title: '{i18n>PO_ITEM_NODE_KEY}');
        PARENT_KEY: Association to purchaseorder @(title: '{i18n>PO_ID_NODE_KEY}');
